...
class FSQAE(Base):
    ...
    def forward(self, x_in, e_in, sampling=False, add_noise=True):
        '''
        @params
            x_in: {0, 1}^(B, N, d_x)
                x: input feat
...
class LatentBridge(object):
    ...
    def __init__(self, 
        ...
        if use_global_embed:
            in_dim_y = data_shape[-1] + 1 # cfg.hid_dim_y
        else:
            in_dim_y = 1 # cfg.hid_dim_y
    ...
    def _get_pertubed_data(self, batch, mask):
        ...
        @params
            batch: {R}^(B, N, F)
        '''
        # TODO: this func consumes on 1 gpu
        ...
        # noise = noise * _mask # TODO uncomment this line.
        ...
        # grad_log_h = torch.zeros(perb_x.shape, device=perb_x.device)
    ...
    def _get_log_h(self, cb, z, beta_T_minus_beta, mask):
        ...
        # sum here to get a scalar result -> autograd possible
        return log_h.log().sum(-1).sum(-1)
        # TODO should put mask after log operator.
        # return log_h.log().sum(-1).mean(-1)
    
    @torch.no_grad()
    def sample(self, n, device, mask):
        '''
        @params:
            n: int
                number of samples.
        ...
        for t in range(self.N):
            ...
            with torch.enable_grad():
                ...
                for i in range(z.shape[1]):
                    ...
                    grad_log_h_i = torch.autograd.grad(log_h_sum, z_i, allow_unused=True, retain_graph=False)[0]
                    grad_log_h[:,i:i+1,:] = grad_log_h_i
            # TODO check if grad_log_h_i is masked
            # grad_log_h = grad_log_h * _mask
            # t = t.view(-1, 1) / self.N # (n, 1)
            t = t.view(-1, 1) # (n, 1)
