...
from .spec_utils import motif_stats, degree_stats, \
spectral_stats, clustering_stats, orbit_stats_all, eval_acc_sbm_graph, eval_acc_planar_graph, eval_fraction_unique_non_isomorphic_valid, \
eval_fraction_isomorphic, is_sbm_graph, is_planar_graph
    ...
    def forward(self, x_gen, adj_gen, logger):
        ...
        @params
            ...
            # adj_gen: {R}^(B, N, N, 4) 
                ...
                generated class probabilities of each class.
        '''
        # adj_gen = adj_gen.argmax(dim=-1)
        # x_gen = x_gen.argmax(dim=-1)
        ...
        # nvun = (num_mols_wo_correction / num_mols)*scores[f'unique@{len(gen_smiles)}']*scores['Novelty']
...
class SpectreSamplingMetric(nn.Module):
    ...
    def forward(self, generated_graphs):
        ...
        if 'spectre' in self.metrics_list:
            ...
            spectre = spectral_stats(self.test_nx_graph, gen_nx_graph, is_parallel=True, n_eigvals=-1,
                                     compute_emd=self.compute_emd)
        ...
        if 'motif' in self.metrics_list:
            ...
            motif = motif_stats(self.test_nx_graph, gen_nx_graph, motif_type='4cycle', ground_truth_match=None, bins=100,
                                compute_emd=self.compute_emd)
            to_log['motif'] = motif
        ...
        if 'sbm' in self.metrics_list:
            ...
            acc = eval_acc_sbm_graph(gen_nx_graph, refinement_steps=100, strict=True)
            to_log['sbm_acc'] = acc

        if 'planar' in self.metrics_list:
            print('Computing planar accuracy...')
            planar_acc = eval_acc_planar_graph(gen_nx_graph)
        ...
        if 'sbm' in self.metrics_list or 'planar' in self.metrics_list:
            ...
            frac_unique, frac_unique_non_isomorphic, fraction_unique_non_isomorphic_valid = eval_fraction_unique_non_isomorphic_valid(
                gen_nx_graph, self.train_nx_graph, is_sbm_graph if 'sbm' in self.metrics_list else is_planar_graph)
            frac_non_isomorphic = 1.0 - eval_fraction_isomorphic(gen_nx_graph, self.train_nx_graph)
