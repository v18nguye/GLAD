    ...
    if dataset == 'QM9':
        def transform(g):
            ...
            if num_atom > 1 and (mask_atom == mask_edge.numpy()).all():
                ...
                indices = x[mask] - 6
                x_[mask, indices] = 1
                # x = x.argmax(dim=-1).to(torch.long) 
...
def process_generic_data(dataset):
    _num_xfeat = {'community': 10 - 1, 'ego': 17 - 1, 'grid': 5 - 1, 'ENZYMES': 10 - 1} # no taking account to zero degree.
    _max_node_num = {'community': 20, 'ego': 18, 'grid': 361, 'ENZYMES': 125}
    
    if dataset in ['community', 'ego']:
        file_path = os.path.join(DATA_DIR, dataset+'_small.pkl')
    elif dataset in ['grid', 'ENZYMES']:
    ...
    for g in raw_list:
        ...
        # x_feat = x_feat.argmax(dim=-1).to(torch.long)
        data = (x_feat, pad_adj)
